    ...
    <MyCart.Provider value={{ isLogin, cartQuantity, setCartQuantity, setIsLogin, cartServer, cartClient, setCartClient, setIsFetch }}>
      ...
      >
        ...
        {role == location.pathname.split("/")[1] ? (
          <Layout className="flex-grow flex flex-col gap-5 py-2">
            <Sider className="!w-64 !max-w-64 !flex-none h-full bg-transparent">
              <SiderPage role={role} />
            </Sider>
            <Content className="h-auto w-full px-12 bg-white rounded-xl">
              <Outlet />
            </Content>
          </Layout >
        ) : (
          <Content
            className={`h-auto flex-grow ${location.pathname.split("/")[1] == "signin" || location.pathname.split("/")[1] == "signup" ? "backgroundMain" : " bg-transparent"
              }`}
          >
            <Outlet />
          </Content >
